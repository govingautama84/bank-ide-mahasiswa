...
$stmt = $pdo->prepare("SELECT * FROM ide WHERE id_ide = ?");
$stmt->execute([$id_ide]);
$ide = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($judul_ide) || empty($deskripsi) || empty($id_kategori)) {
        ...
        $stmt = $pdo->prepare("UPDATE ide SET judul_ide = ?, deskripsi = ?, id_kategori = ? WHERE id_ide = ?");
        if ($stmt->execute([$judul_ide, $deskripsi, $id_kategori, $id_ide])) {
            ...
            $error = 'Terjadi kesalahan saat mengupdate ide.';
        }
    }
}
?>
        ...
        <div class="card">
            <form method="POST" action="">
                ...
                <div class="flex gap-1 mt-2">
                    <button type="submit" class="btn btn-primary">Update Ide</button>
